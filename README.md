...
results/
  <sample_ID>_<program>/
    ...
    filtered_bams/
      <sample_ID>.bam            # Filtered BAM output files and indexes
      <sample_ID>.bam.bai
    raw_bams/
      <sample_ID>.bam            # Raw BAM output by mapper (when --publish_raw_bam true)
...
| `--publish_raw_bam`    | `boolean` | `false` | Publish raw bam files (no indexes), as generated by mappers (and conversion to BAM format). Only filtered bam files will be output by default." |
