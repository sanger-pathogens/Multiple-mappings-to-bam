...
def menu_system(option):
    ...
    while not run:
        ...
        if option.ref == '':
            ...
            print("File: Paths containing the files to be mapped:\t\t" +
                  str(option.mapfiles))
            print("r: Reference dna sequence:\t\t" + option.ref)
            print(
                "outdir: Output directory name to dump all the outputs:\t\t" + options.outdir)
            ...
            print("S: Minimum ssaha quality score:\t\t" +
                  str(option.ssahaquality))
            ...
            print("2: Enable read-pair overlap detection:\t" +
                  str(option.detectOverlaps))
            print("X: Create pseudosequences:\t\t" + str(option.pseudosequence))
            print("x: Include reference in pseudosequence alignment:\t" +
                  str(option.incref))
            print(
                "I: Include small indels in pseudosequence alignment:\t" + str(option.indels))
            ...
            print("D: Minimum number of reads matching SNP per strand:\t" +
                  str(option.stranddepth))
            print("A: Use anomolous reads in mpileup:\t" + str(option.anomolous))
            print(
                "B: Use samtools base alignment quality option (BAQ):\t" + str(option.BAQ))
            ...
            print(
                "k: If old mapping files are present, do not rerun them:\t" + str(option.keep))
            ...
            print(
                "n: Maximum number of jobs to run on nodes in parallel:\t" + str(option.nodes))
            print("y: Do not clean up temporary files:\t" + str(option.dirty))
            print(
                "docker: Run on docker containers (Default singularity):\t\t" + str(option.docker))
        print("\nquit: QUIT")
        if option.ref == "":
        ...
        if ui == 'y':
            ...
            while True:
                ref_file_path = input(
                    'Enter reference file name including path or Q to go back to the menu: ')
                print(ref_file_path)
            ...
            while True:
                inp = input(
                    'Enter program (bwa, ssaha, smalt) or Q to go back to the menu: ').lower()
                if inp == 'q':
                    break
            ...
            while True:
                inp = input(
                    'Enter true or false or Q to go back to the menu: ').lower()
                if inp == 'q':
                    break
            ...
            while True:
                inp = input(
                    'Enter maximum insert size (10-10,000) or Q to go back to the menu. Must be more than min: ').lower()
                if inp == 'q':
                    break
            ...
            while True:
                inp = input(
                    'Enter minimum insert size (10-10,000) or Q to go back to the menu. Must be less than max: ').lower()
                if inp == 'q':
                    break
            ...
            if option.program != 'ssaha':
                print(
                    "ssaha quality score is only applicable for ssaha program. Press any key to continue.")
                inp = input()
            while True:
                inp = input(
                    'Enter minimum ssaha quality score while mapping (ssaha only) or Q to go back to the menu: ').lower()
                if inp == 'q':
                    break
                elif int(inp) >= 0:
            ...
            option.nomapid = float(
                input('Enter minimum identity threshold to report a mapping: '))
        elif ui == 'G':
            while True:
                inp = input(
                    'Enter true or false or Q to go back to the menu: ').lower()
            ...
            while True:
                ...
                if inp == 'q':
                    ...
                    if option.program == 'bwa':
                        print(
                            "Mapping quality score (-Q) must be between 0 and 30 for BWA.")
                    else:
                        print(
                            f"Mapping quality score (-Q) must be between 0 and 60 for {option.program}")
        elif ui == 'd':
            while True:
                inp = input(
                    'Enter minimum number of reads matching SNP: ').lower()
                if inp == 'q':
                    break
                elif int(inp) >= 0:
                    option.depth = inp
            ...
            while True:
                inp = input(
                    'Enter minimum number of reads matching SNP per strand: ').lower()
                if inp == 'q':
                    break
                elif int(inp) >= 0:
            ...
            while True:
                ...
                if inp > 1 or inp < 0:
                    print(
                        "SNP/site mapping quality ratio cutoff (-R) must be between 0 and 1")
                else:
                    option.ratio = inp
        elif ui == 'P':
            while True:
            ...
            while True:
                inp = input(
                    'Enter bcftools caller (c or m) or Q to go back to the menu: ').lower()
                if inp == 'q':
                    break
                elif inp in ['c', 'm']:
                    option.call = inp
            ...
            inp = input(
                'Enter reference annotation or Q to go back to the menu: ')
            if inp != 'Q':
                option.embl = inp
        elif ui == 'o':
            inp = input(
                'Enter output file prefix or Q to go back to the menu: ')
            if inp != 'Q':
                option.output = inp
        elif ui == 'O':
            inp = input(
                'Enter output directory suffix or Q to go back to the menu: ')
            if inp != 'Q':
            ...
            while True:
                inp = input(
                    'Enter filter or split bam file (1, 2, 3, 4, 5) or Q to go back to the menu: ').lower()
                print(inp)
                if inp == 'q':
                    break
            ...
            while True:
                inp = input(
                    'Enter true or false or Q to go back to the menu: ')
                if inp == 'Q':
                    break
            ...
            while True:
                inp = input(
                    'Enter LSF queue to submit to (normal, long, basement, hugemem) or Q to go back to the menu: ').lower()
                if inp == 'q':
                    break
            ...
            inp = input(
                'Enter maximum number of jobs to run on nodes in parallel: ')
            if inp == 'Q':
                continue
            if inp >= 0:
                option.nodes = inp
...
def validate_inputs(options):
    ...
    if options.output == '':
        options.output = options.ref.split(
            "/")[-1].split(".")[0]+"_"+options.program
