...
from Bio.Alphabet import IUPAC, Gapped, SingleLetterAlphabet, generic_dna
...
def main():
    ...
    parser.add_option("-o", "--output", action="store",
                      dest="output", help="output file prefix", default="")
# parser.add_option("-m", "--muscle", action="store_true", dest="muscle", help="Try to align regions with muscle [default=%default]", default=False)
    ...
    parser.add_option("-t", "--textfile", action="store", dest="textfile",
                      help="textfile containing input mfa files", default="")
...
if __name__ == "__main__":
    ...
    for sequence in fasta:
        ...
        refseq[sequence.id].id = '.'.join(
            options.ref.split("/")[-1].split(".")[:-1])
            ...
            for sequence in sequences[contig]:
                ...
                if Deletions[contig][location].has_key(sequence):
                    ...
                    if distfromstart+deletionlen < len(tempseqs[-1].seq):
                        endbit = tempseqs[-1].seq[distfromstart +
                                                  1+deletionlen:].upper()
                    else:
                        endbit = ""
...
# sys.exit()
            ...
            for line in muscleout:
                ...
                if not sequences[contig].has_key(name):
                    refseq[contig].seq = refseq[contig].seq[:start] + \
                        ''.join(words[1:])+refseq[contig].seq[end:]
                    continue
                else:
                    ...
                    sequences[contig][name].seq = sequences[contig][name].seq[:start] + \
                        ''.join(words[1:])+sequences[contig][name].seq[end:]
...
# tempseqs[-1].id=tmpname
            ...
            for sequence in sequences[contig].keys():
                ...
                if Insertions[contig][location].has_key(sequence):
                    tempseqs[-1].seq = tempseqs[-1].seq[:align_size+1] + \
                        Insertions[contig][location][sequence] + \
                        tempseqs[-1].seq[align_size+1:]


# if options.muscle:
            SeqIO.write(tempseqs, open(tmpname+".fasta", "w"), "fasta")
            os.system("muscle -in "+tmpname+".fasta -out " +
                      tmpname+".aln  >  /dev/null 2>&1 ")
# else:
# SeqIO.write(tempseqs, open(tmpname+".aln","w"), "fasta")
...
# os.system("seaview "+tmpname+".aln")
                ...
                if not sequences[contig].has_key(name):
                    ...
                    while ("N-" in seqline) or ("-N" in seqline):
                        seqline = seqline.replace(
                            "N-", "NN").replace("-N", "NN")
                    sequences[contig][name].seq = sequences[contig][name].seq[:start] + \
                        seqline+sequences[contig][name].seq[end:]
    ...
    for sequence in sequences[contig].keys():
        ...
        for contig in reforder[1:]:
            final_sequences[-1].seq = final_sequences[-1].seq + \
                sequences[contig][sequence].seq
