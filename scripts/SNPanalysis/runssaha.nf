...
process runSsaha {
    ...
    """
        ...
        if [ "${pairedend}" = "false" ]; then
            echo "ssaha2 -score ${ssahaquality} -kmer 13 -skip 2 -seeds 2 -score 12 -cmatch 9 -ckmer 6 -diff 0 -output sam_soft -outfile ${runname}/tmp1.sam ${ref} ${fastqdir}${name}.fastq" >> ${bashfile}
        else
            echo "ssaha2 -score ${ssahaquality} -kmer 13 -skip 2 -seeds 2 -score 12 -cmatch 9 -ckmer 6 -diff 0 -outfile ${runname}/tmp1.sam -pair ${mininsertsize},${maxinsertsize} -output sam_soft ${ref} ${fastqdir}${name}_1.fastq ${fastqdir}${name}_2.fastq" >> ${bashfile}
        fi

        echo "samtools view -b -S ${runname}/tmp1.sam -t ${ref}.fai > ${runname}/tmp1.bam" >> ${bashfile}

        if [ "${pairedend}" = "true" ] && [ "${circular}" = "true" ]; then
