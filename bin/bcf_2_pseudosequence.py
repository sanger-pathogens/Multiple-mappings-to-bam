...
def main():
    ...
    group.add_option("-b", "--bcf", action="store", dest="bcf",
                     help="bcf/vcf file", default="", metavar="file")
    group.add_option("-v", "--vcf", action="store_true", dest="vcf",
                     help="variation input file is in vcf format [default is bcf]", default=False)
    group.add_option("-B", "--bam", action="store", dest="bam",
                     help="bam/sam file", default="", metavar="file")
    group.add_option("-s", "--sam", action="store_true", dest="sam",
                     help="bam/sam input file is in sam format [default is bam]", default=False)
    group.add_option("-o", "--output", action="store", dest="output",
                     help="prefix for output files", default="", metavar="prefix")
# group.add_option("-p", "--pseudosequence", action="store", dest="pseudosequence", help="output pseudosequence including indels [default is to write a psedoalignment to the reference and separate indel text file]", default=False)
    ...
    group.add_option("-d", "--depth", action="store", dest="depth",
                     help="Minimum number of reads matching SNP [default= %default]", default=4, type="int", metavar="int")
    group.add_option("-D", "--stranddepth", action="store", dest="stranddepth",
                     help="Minimum number of reads matching SNP per strand [default= %default]", default=2, type="int", metavar="int")
    group.add_option("-r", "--ratio", action="store", dest="ratio",
                     help="Minimum ratio of first to second base call [default= %default]", default=0.8, type="float", metavar="float")
    # are the three above options still necessary???
    group.add_option("-q", "--QUAL", action="store", dest="QUAL",
                     help="Minimum base quality [default= %default]", default=50.0, type="float", metavar="float")
    group.add_option("-m", "--MQUAL", action="store", dest="MQUAL",
                     help="Minimum mapping quality [default= %default]", default=0.0, type="float", metavar="float")
    group.add_option("-a", "--AF1", action="store", dest="AF1",
                     help="Minimum allele frequency (you would expect an AF of 1 for haploid SNPs). For non-SNP bases, the program will use 1- this number. [default= %default]", default=0.95, type="float")
    ...
    group.add_option("-c", "--CI95", action="store", dest="CI95",
                     help="Maximum 95% confidence interval variation from AF. [default= %default]", default=0.0, type="float", metavar="float")
    group.add_option("-S", "--strandbias", action="store", dest="strand_bias",
                     help="p-value cutoff for strand bias. [default= %default]", default=0.001, type="float", metavar="float")
    group.add_option("-Q", "--baseqbias", action="store", dest="baseq_bias",
                     help="p-value cutoff for base quality bias. [default= %default]", default=0.0, type="float", metavar="float")
    group.add_option("-M", "--mappingbias", action="store", dest="mapping_bias",
                     help="p-value cutoff for mapping bias. [default= %default]", default=0.001, type="float", metavar="float")
    group.add_option("-T", "--taildistancebias", action="store", dest="tail_bias",
                     help="p-value cutoff for tail distance bias. [default= %default]", default=0.001, type="float", metavar="float")
...
if __name__ == "__main__":
    ...
    if options.ratio < 0.5 or options.ratio > 1:
        DoError(
            "Ratio of first to second base (-r) must be greater than 0.5 and less than or equal to 1")
    ...
    if options.CI95 < 0 or options.CI95 > 1:
        DoError(
            "Maximum 95% confidence interval of allele frequency (-c) must be between 0 and 1")
                ...
                for i in info:
                    ...
                    if len(i.split("=")) < 2:
                        ...
                        try:
                            ...
                            try:
                                BASEINFO[heading][infotype] = map(
                                    float, infodata.split(","))
                            except StandardError:
...
# continue
        ...
        if not "DP4" in BASEINFO["INFO"]:
            ...
            try:
                BASEINFO["INFO"]["DP4ratios"]["fref"] = float(
                    BASEINFO["INFO"]["DP4"][0])/(BASEINFO["INFO"]["DP4"][0]+BASEINFO["INFO"]["DP4"][2])
            except ZeroDivisionError:
                BASEINFO["INFO"]["DP4ratios"]["fref"] = 0.0
            try:
                BASEINFO["INFO"]["DP4ratios"]["rref"] = float(
                    BASEINFO["INFO"]["DP4"][1])/(BASEINFO["INFO"]["DP4"][1]+BASEINFO["INFO"]["DP4"][3])
            except ZeroDivisionError:
                BASEINFO["INFO"]["DP4ratios"]["rref"] = 0.0
            try:
                BASEINFO["INFO"]["DP4ratios"]["falt"] = float(
                    BASEINFO["INFO"]["DP4"][2])/(BASEINFO["INFO"]["DP4"][0]+BASEINFO["INFO"]["DP4"][2])
            except ZeroDivisionError:
                BASEINFO["INFO"]["DP4ratios"]["falt"] = 0.0
            try:
                BASEINFO["INFO"]["DP4ratios"]["ralt"] = float(
                    BASEINFO["INFO"]["DP4"][3])/(BASEINFO["INFO"]["DP4"][1]+BASEINFO["INFO"]["DP4"][3])
            except ZeroDivisionError:
                BASEINFO["INFO"]["DP4ratios"]["ralt"] = 0.0
            try:
                BASEINFO["INFO"]["DP4rratio"] = (float(BASEINFO["INFO"]["DP4"][0])+float(BASEINFO["INFO"]["DP4"][1]))/(
                    BASEINFO["INFO"]["DP4"][0]+BASEINFO["INFO"]["DP4"][1]+BASEINFO["INFO"]["DP4"][2]+BASEINFO["INFO"]["DP4"][3])
            except ZeroDivisionError:
                BASEINFO["INFO"]["DP4rratio"] = 1.0
...
# print >> vcf, '\t'.join(map(str,vcfline))
        ...
        if keep:
            ...
            # if the previous base was an indel, this base is mapped, so print the indel to the file
            if len(addindel) > 0:
            ...
            if SNP:
                ...
                snps += 1
                # if it's an indel check the previous base has been mapped
                if INDEL and contigs[BASEINFO["CHROM"]][int(BASEINFO["POS"])-1] != "N":
                    ...
                    if len(BASEINFO["REF"]) > len(BASEINFO["ALT"]) and (len(BASEINFO["ALT"]) == 1 or BASEINFO["REF"][1:].endswith(BASEINFO["ALT"][1:])):
                        if len(BASEINFO["ALT"]) == 1:
                            ...
                            deletion = BASEINFO["REF"][1:0 -
                                                       (len(BASEINFO["ALT"])-1)]

                        addindel = [BASEINFO["CHROM"], int(
                            BASEINFO["POS"]), "-", deletion]
                        ...
                        if len(BASEINFO["REF"]) == 1:
                            ...
                            insertion = BASEINFO["ALT"][1:0 -
                                                        (len(BASEINFO["REF"])-1)]

                        addindel = [BASEINFO["CHROM"], int(
                            BASEINFO["POS"]), "+", insertion]
                        # indels.append([BASEINFO["CHROM"], int(BASEINFO["POS"]), "+", insertion])

                else:
                    contigs[BASEINFO["CHROM"]][int(
                        BASEINFO["POS"])-1] = BASEINFO["ALT"][0]
                    mapped += 1

            elif not INDEL:
                contigs[BASEINFO["CHROM"]][int(
                    BASEINFO["POS"])-1] = BASEINFO["REF"][0]
                mapped += 1
                snpline = 0

            print >> mapplot, int(BASEINFO["POS"]), int(
                BASEINFO["INFO"]["DP"]), snpline
...
# vcf.close()
    ...
    print >> out, 'Mapped:%d (%.2f%%)' % (
        mapped, (float(mapped)/totallength)*100)
    print >> out, 'SNPs:', snps
    print >> out, 'Insertions:', insertions, "(" + \
        str(insertion_lengths)+" bases)"
